...
【句子标记】只在「文章展示」正文里，用 *斜体* 标出两类值得学的句子（斜体句后不加任何 [^n]）：
...
- 斜体只在正文里就地标注，**不要**在文末或任何位置再单独罗列成清单。
- **一致性要求（重要）**：凡是后面「句子赏析」「长难句语法精讲」「写作手法·原文呈现」会用到的句子，都必须先在正文用 *斜体* 标好，一一对应、绝不遗漏（原文标了＝后面会讲，后面讲了＝原文一定有标）。
- **但在后面各分析板块里引用这些句子时（原句 / 原文呈现），一律用普通文本，不要再加斜体或任何强调。斜体只出现在「文章展示」中。**
...
## 三、长难句语法精讲
二级标题“## 🧩 长难句语法精讲”。挑出文中**较难读懂或有代表性的长难句**（长句、各类从句、非谓语、倒装、强调、虚拟、插入、省略等），重在帮读者读懂，与“句子赏析”挑的漂亮句尽量错开、不重复。每句用有序列表的一项，包含（引用原句用普通文本、不加斜体）：
- **原句**：...
- **主干**：去掉修饰后的核心 S+V+O
...
## 四、句子赏析
二级标题“## 🖋️ 精彩句子赏析”。逐一赏析前面用 *斜体* 标出的每一个句子。每个句子用有序列表的一项，包含三行（引用原句用普通文本、不加斜体）：
- **原句**：...
...
def deepdive():
    ...
    note = lang_note(data.get("lang"))
    if note:
        messages.append({"role": "system", "content": note + "（深度思考请用这门语言书写。）"})
