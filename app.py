# -*- coding: utf-8 -*-
# 外刊阅读助手 (Journal Reading Helper)
# Copyright (c) 2026 Tj  <https://github.com/Tristan0929>
...
【句子标记】只在「文章展示」正文里，用 ==两个等号== 把值得学的句子包起来（例如：==This is a great sentence.== 或 ==これは名文です。==）。**务必用 == == 而不是斜体**，因为斜体对中日韩不生效。要标两类句子：
① **精彩句子**——后面「句子赏析」要讲的：修辞（比喻/类比/拟人/对照/平行/反复/递进）、句法精巧（倒装/破折号收尾/插入语/圆周句/松散句/同位语堆叠）、警句格言式、生动意象或有节奏感；宁精勿滥、覆盖开头中段结尾、去掉雷同。
② **结构精妙的长难句**——后面「长难句语法精讲」要拆解的。
- == == 只在正文里就地标注，**不要**在文末或任何位置再单独罗列成清单。
- **一致性要求（重要）**：凡是后面「句子赏析」「长难句语法精讲」「写作手法·原文呈现」会用到的句子，都必须先在正文用 == == 标好，一一对应、绝不遗漏（原文标了＝后面会讲，后面讲了＝原文一定有标）。
- **但在后面各分析板块里引用这些句子时（原句 / 原文呈现），一律用普通文本，不要加 == ==、不要斜体或任何强调。== == 只出现在「文章展示」中。**
...
## 三、长难句语法精讲
二级标题“## 🧩 长难句语法精讲”。挑出文中**较难读懂或有代表性的长难句**（长句、各类从句、非谓语、倒装、强调、虚拟、插入、省略等），重在帮读者读懂，与“句子赏析”挑的漂亮句尽量错开、不重复。
用有序列表，每句是**一个序号项**；项内四行用 <br> 分隔、**行与行之间不空行**；不同序号之间空一行。引用原句用普通文本（不加 == ==、不加斜体）。格式：
1. **原句**：...<br>**主干**：去掉修饰后的核心主谓宾<br>**结构拆解**：标出各成分（主句/从句、非谓语、插入语、省略、倒装、强调等）并点明语法点<br>**参考译文**：...

## 四、句子赏析
二级标题“## 🖋️ 精彩句子赏析”。逐一赏析前面用 == == 标出的每一个精彩句子。
用有序列表，每句是**一个序号项**；项内三行用 <br> 分隔、**行与行之间不空行**；不同序号之间空一行。引用原句用普通文本（不加 == ==、不加斜体）。格式：
1. **原句**：...<br>**妙处**：...（结构、修辞、用词、节奏等，简明点出）<br>**仿写**：...（模仿其特点写一个完整句，用原文的语言）

## 五、写作手法分析
二级标题“## ✍️ 写作手法仿写”。选出文中突出的写作技巧，每个技巧的名称用三级标题（### 技巧名称）开头，其下包含：
- **原文呈现**：引用文中句子（用普通文本，不要斜体或加粗）
- **效用分析**：它为什么好
...
DEEPDIVE_SYSTEM_PROMPT = """IMPORTANT: Write your ENTIRE response in the SAME language as the article below. If the article is in English, write in English; if it is in Japanese, write in Japanese; if German, write in German, etc. Do NOT write in Chinese (unless the article itself is Chinese).

You are a sharp, well-read current-affairs analyst and critical thinker. The user is studying a foreign-language article and wants to think about it more deeply, in that language.

Write an original, thought-provoking analysis. Do NOT merely summarize. Instead:
...
Reminder: the entire response MUST be in the same language as the article (not Chinese, unless the article is Chinese)."""

TRANSLATE_SYSTEM_PROMPT = """把用户给的文本翻译成自然、通顺的中文。只输出中文译文，不要原文、不要注释、不要任何多余的话。"""

DEFINE_SYSTEM_PROMPT = """你是词汇卡片助手。用户给你一个外语词或词组（可能附上下文），请按背诵速查表的风格给出学习卡片。
严格只输出 JSON，不要任何多余文字、不要代码块包裹：
{"def":"中文释义（简洁准确，可点出为什么地道）","scene":"使用场景（简短，如：正式书面 / 口语 / 商业 等）"}"""

DISCUSS_SYSTEM_PROMPT = """You are the user's discussion partner and writing coach. The user is a learner practicing writing by discussing the topic below with you. Reply in the SAME language as the analysis/article being discussed (English article -> English; Japanese -> Japanese; etc.), NOT Chinese.

For each of the user's messages, do TWO things, in this order:
1. **Polish** — If the user's writing is awkward, unclear or grammatically off, briefly show a smoother, more natural version in that language. Start this line with "✍️ Polished: ...". Keep it short; if the message is already fine, skip this or just note it reads well.
...
def deepdive():
    ...
    outlang = (data.get("outlang") or "").strip()
    if outlang and outlang.lower() not in ("auto", "跟随原文", "follow"):
        messages.append({"role": "system",
                         "content": "OVERRIDE: Write your ENTIRE response in %s, regardless of the article's language." % outlang})
    messages.append({"role": "user", "content": article[:9000]})
...
@app.route("/defineword", methods=["POST"])
def defineword():
    data = request.get_json(force=True)
    api_key = (data.get("api_key") or "").strip()
    base_url = (data.get("base_url") or "").strip()
    ...
    try:
        resp = client.chat.completions.create(
            model=model,
            messages=[{"role": "system", "content": DEFINE_SYSTEM_PROMPT},
        ...
        if not raw.startswith("{"):
            ...
            if m:
                raw = m.group(0)
        parsed = json.loads(raw)
        return jsonify({"def": parsed.get("def", ""), "scene": parsed.get("scene", "")})
    except json.JSONDecodeError:
